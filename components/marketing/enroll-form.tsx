...
export function EnrollForm({
  ...
  async function handleSubmit(e: React.FormEvent<HTMLFormElement>) {
    ...
    if (isMinor) {
      payload.parent_name = form.get("parent_name") || null;
      payload.parent_phone = form.get("parent_phone") || null;
    }
  ...
  if (status === "success") {
    return (
      <>
        <PageBanner title="Thank You!" subtitle="Your enrollment application has been received." imageUrl={successBannerUrl} />
          ...
          <form onSubmit={handleSubmit} className="space-y-6 rounded-xl border border-blue/20 bg-kaizen-dark p-8">
            ...
            <div className="space-y-1.5">
              ...
              <select
                ...
                required
                className="flex h-11 w-full rounded-md border border-blue/20 bg-kaizen-black px-3 text-sm text-kaizen-silver"
                defaultValue=""
              >
            ...
            {isMinor && (
              <details open className="rounded-lg border border-blue/20 bg-kaizen-black/50 p-4">
