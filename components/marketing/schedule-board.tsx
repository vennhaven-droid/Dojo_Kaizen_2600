...
import { WEEKLY_SCHEDULE_BLOCKS } from "@/lib/brand";
...
function ScheduleRow({ entry, index }: { entry: ScheduleEntry; index: number }) {
  return (
    ...
    >
      ...
      <p className="font-display font-bold text-kaizen-gray transition-colors group-hover:text-gold">
        {entry.className}
      </p>
...
export function ScheduleBoard({ scheduleEntries }: ScheduleBoardProps) {
  const useDbSchedule = Boolean(scheduleEntries?.length);

  return (
    <div className="space-y-16">
      {useDbSchedule ? (
        <FadeIn>
          <section className="overflow-hidden rounded-2xl border border-blue/30">
            <div className="border-b border-white/5 bg-kaizen-black/60 px-6 py-5 sm:px-8">
              <h2 className="font-display text-xl font-bold text-kaizen-gray sm:text-2xl">Class Schedule</h2>
              <p className="mt-1 text-sm text-kaizen-muted">Updated from academy schedule</p>
            </div>
            <ul className="divide-y divide-white/5">
              {scheduleEntries!.map((entry, i) => (
                <ScheduleRow key={`${entry.time}-${entry.className}-${i}`} entry={entry} index={i} />
              ))}
            </ul>
          </section>
        </FadeIn>
                ...
                <ul className="divide-y divide-white/5">
                  {block.entries.map((entry, i) => (
                    <ScheduleRow
                      ...
                      entry={{ time: entry.time, className: entry.className, days: entry.days }}
                      index={i}
                    />
                  ))}
                </ul>
